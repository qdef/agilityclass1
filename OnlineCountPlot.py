...
cursor.execute("CREATE TABLE IF NOT EXISTS TwitterAD (Count bigserial primary key, followers integer, time varchar(8), date date);")

while True:
	website = requests.get('https://twitter.com/BlackSabbath?lang=fr')
	...
	cursor.execute("select max(count) from TwitterAD")
	...
	cursor.execute("INSERT INTO TwitterAD (time, date, followers) VALUES (%s, %s, %s);", (CurrTime, CurrDate, followers))
	conn.commit()
	time.sleep(900)
conn.close()
